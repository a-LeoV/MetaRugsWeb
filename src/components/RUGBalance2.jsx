  ...
  return (
    <>
      ...
      <div className="home container">
        <div className="startup" style={{ margin: "20px 0" }}>
          <h1>Congratulations</h1>
          <h2>
            You've been provably rugged{" "}
            <span className="startup_blue">
              {" "}
              {totalNFTs > 0 ? totalNFTs : "undefined"}{" "}
            </span>{" "}
            times
          </h2>
          <div></div>
          <h2>
            You can mint a <span className="startup_blue"> MetaRug </span> for
            each CryptoRug you burn.
          </h2>
        </div>
      </div>
      <div className="burnbutton_group">
        <div className="burn">
          <h2>Enjoy getting rugged?</h2>

          <button
            className="blue"
            onClick={() =>
              window.open(
                "https://opensea.io/collection/thecryptorugs",
                "_blank"
              )
            }
          >
            {" "}
            CryptoRugs on OpenSea{" "}
          </button>
        </div>
      </div>

      <div className="rug_card">
        <h1 className=" h1_title">
          {" "}
          {"Save gas and burn all your CryptoRugs at once!"}{" "}
        </h1>
        <NavLink className="navLink" to="/burn">
          🔥 Burn All your CryptoRugs & Mint MetaRugs
        </NavLink>
      </div>
      <div>
        <div style={styles.NFTs}>
          ...
          {RUGBalance &&
            RUGBalance.map((nft, index) => (
              <Card
                hoverable
                onClick={() => handleBurn2Mint_ONE_RUG(nft)}
                actions={[
                  <Tooltip title="View On Blockexplorer">
                    <FileSearchOutlined
                      onClick={() =>
                        window.open(
                          `${getExplorer(chainId)}token/${
                            nft.token_address
                          }?a=${nft.token_id}`,
                          "_blank"
                        )
                ...
                style={{ width: 326, border: "2px solid #e7eaf3" }}
                cover={
                  <Image
                    preview={false}
                    src={nft?.image || "error"}
        ...
        <Modal
          title={`Burn ${
            nftToBurn?.name + " #" + nftToBurn?.token_id || "NFT"
          }`}
          visible={visible}
